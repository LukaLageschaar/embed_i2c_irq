 ...
 * 		Sets the base addres of the UART registers and creates memory space
 * 		to access data after an interrupt has been handled.
 */
int uart_init()
{
    UART *up;
 ...
 * 	  Checks which UART interrupt was set then handles accordingly
 * 
 * @param UART* pointer to the address of the UART struct at memory space &uart
 */
 ...
 * 
 * @param UART* pointer to the address of the UART struct at memory space &uart
 * @param char  The character that is to be put on the uart bus
 */
 ...
 * 	  makes use of the uputc and ugetc functions
 * 
 * @param UART* pointer to the address of the UART struct at memory space &uart
 * @param Char* Pointer to the address space where you want to store the received line of characters
 */
int ugets(UART *up, char *s) 
{
    int size =0;
    while ((*s = (char)ugetc(up)) != '\r'){ //untill enter is typed (\r = carriage return) add characters to the pointer
	size++;
	uputc(up, *s++); // This is done to display the char you just typed in a Uart terminal
	// else you would be typing and nothing would display on the screen
    }
    *s = 0;
    uprintf("\n\r");
    return size;
 ...
 * 	  makes use of the uputc function
 * 
 * @param UART* pointer to the address of the UART struct at memory space &uart
 * @param Char* pointer to the array of characters you want to print 
 */
